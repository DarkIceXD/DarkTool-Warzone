...
namespace decryption {
    extern "C" auto decrypt_client_info(uint64_t imageBase, uint64_t peb) -> uint64_t
    {
        ...
        RBX = readMemory<uint64_t>(imageBase + 0x17755488);
        if (RBX == 0) {
            return 0;
        }
        RAX = imageBase;
        ...
        RAX ^= RDX;
        RCX ^= readMemory<uint64_t>(imageBase + 0x71ED100);
        RCX = ~RCX;
    ...
    extern "C" auto decrypt_client_base(uint64_t clientInfo, uint64_t imageBase, uint64_t peb) -> uint64_t
    {
        ...
        RBX = ~RBX;
        // test rax,rax
        // je 00007FF691AE6F2Eh
        RCX = RBX;
        RCX >>= 0x12;
        RCX &= 0xF;
        // cmp rcx,0Eh
        // ja 00007FF691AE6A66h
        switch (RCX) {
        case 0: {
            R11 = readMemory<uint64_t>(imageBase + 0x71ED12B);
        ...
        case 1: {
            ...
            R10 = readMemory<uint64_t>(imageBase + 0x71ED12B);
            RCX = RAX;
            RCX >>= 0xB;
            RAX ^= RCX;
            RCX = RAX;
        ...
        case 4: {
            ...
            R9 = readMemory<uint64_t>(imageBase + 0x71ED12B);
            RCX = imageBase;
            RAX -= RCX;
    ...
    extern "C" auto decrypt_bone_base(uint64_t imageBase, uint64_t peb) -> uint64_t
    {
        ...
        R8 = readMemory<uint64_t>(imageBase + 0x15A3E508);
        if (R8 == 0) {
            return 0;
        }
        RBX = peb; // mov rbx,gs:[rax]
        // test r8,r8
        // je 00007FF691C4C17Eh
        RAX = RBX;
        RAX <<= 0x2B;
        RAX = _byteswap_uint64(RAX);
        RAX &= 0xF;
        // cmp rax,0Eh
        // ja 00007FF691C4BD4Ah
        switch (RAX) {
        case 0: {
            // pushfq
            // push rbx
            ...
            RCX = readMemory<uint64_t>(imageBase + 0x71ED216);
        ...
        case 1: {
            R10 = readMemory<uint64_t>(imageBase + 0x71ED216);
            RAX = R8;
            RAX >>= 0x9;
            R8 ^= RAX;
            RAX = R8;
        ...
        case 4: {
            ...
            R9 = readMemory<uint64_t>(imageBase + 0x71ED216);
            RAX = R8;
            RAX >>= 0xA;
            R8 ^= RAX;
            RAX = R8;
        ...
        case 5: {
            R11 = readMemory<uint64_t>(imageBase + 0x71ED216);
            R8 = R8 + RBX * 2;
    ...
    extern "C" auto get_bone_index(uint32_t index, uint64_t imageBase) -> uint64_t
    {
        ...
        RCX = R8 * 0x2B6;
        RCX -= RAX;
        RAX = readMemory<uint16_t>(RCX + R11 + 0x71FBC10);
        R8 = RAX * 0x13C8;
        RAX = R10;
        RAX = _umul128(RAX, R8, &RDX);
        RAX = R10;
        ...
        RAX = R9 * 0x7CC;
        RAX -= RCX;
        R15 = readMemory<uint16_t>(RAX + R11 + 0x7202580);
        return R15;
    }
}
