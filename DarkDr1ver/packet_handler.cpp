...
static uint64_t copy_memory(const PacketCopyMemory& packet)
{
...
static uint64_t get_base_address(const PacketGetBaseAddress& packet)
{
...
uint64_t packet_handler::handle(const Packet& packet)
{
	...
	case PacketType::packet_get_base_address:
		return get_base_address(packet.data.get_base_address);
...
bool packet_handler::complete_request(const SOCKET client_connection, const uint64_t result)
{
	Packet packet{ };
